...
def model_parser(model, fixed_weight=False, dropout_rate=0.0, bayesian = False):
    ...
    if model == 'Googlenet':
        ...
        base_model = timm.create_model('vit_base_patch16_224.dino', pretrained=True, num_classes=0)
        network = ViTNet(base_model, fixed_weight)
    else:
...
class ViTNet(nn.Module):
    def __init__(self, base_model, fixed_weight=False, dropout_rate=0.0):
        ...
        self.blocks = base_model.blocks #nn.Sequential(*list(base_model.children()))
        self.patch_embed = base_model.patch_embed
        self.pos_drop = base_model.pos_drop
        self.patch_drop = base_model.patch_drop
