...
class DiFormer(nn.Module):
    ...
    def forward_transformers(self, data):
        ...
        The forward pass expects a dictionary with key-value 'img' -- which consists of:
        ...
        local_descs_t = self.transformer_t(self.input_proj_t(src_t), 
                                           self.query_embed_t.weight, pos[0])[0][0]
        local_descs_rot = self.transformer_rot(self.input_proj_rot(src_rot),
                                                self.query_embed_rot.weight, pos[1])[0][0]
    ...
    def forward_heads(self, transformers_res):
        ...
        max_indices = transformers_res.get('max_indices')
